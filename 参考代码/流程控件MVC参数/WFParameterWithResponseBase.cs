    ...
    public abstract class WFParameterWithResponseBase : WFParameterBase, IWFOperation<ResponseData>
    {
        ...
        public ResponseData Execute(Func<Exception, bool> compensation)
        {
            ...
            if (this.CanOperate() == false)
                throw new InvalidOperationException("不能执行操作");
            ...
            try
            {
                ...
                this.EMailCollector.IsTenantMode = true;
                if (this.TenantCode == Consts.DefaultTenantCode || this.TenantCode == Consts.DefaultPlatTenantCode)
                    this.EMailCollector.IsTenantMode = false;
        ...
        protected void DefaultFillEmailCollector(Dictionary<string, object> dic)
        {
            ...
            MailCollectorArg collectorArgs = this.GetMailCollectorArgs();
