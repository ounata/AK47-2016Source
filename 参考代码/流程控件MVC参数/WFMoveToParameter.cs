...
namespace CIIC.HSR.TSP.WF.UI.Control.Interfaces
{
	...
	public class WFMoveToParameter : WFParameterWithResponseBase
	{
        ...
        {
            MailCollectorArg arg = base.GetMailCollectorArgs();
            if (string.IsNullOrEmpty(this.EMailCollector.MailCompletedArguments.AlarmTypeCode) == false)
                arg.CompletedAlarmTypeCode = this.EMailCollector.MailCompletedArguments.AlarmTypeCode;
            if (string.IsNullOrEmpty(this.EMailCollector.MailTaskArguments.AlarmTypeCode) == false)
                arg.TaskAlarmTypeCode = this.EMailCollector.MailTaskArguments.AlarmTypeCode;
            return arg;
        }
        ...
        {
            return HttpContext.Current.Request.GetWFContext().CanMoveTo();
        }
	}
}
