...
namespace PermissionCenter
{
    ...
    public partial class AppRoleMembersView : System.Web.UI.Page
    {
        ...
        protected void ProcessCaculating(object sender, MCS.Web.WebControls.PostProgressDoPostedDataEventArgs e)
        {
            ...
            e.Result.ProcessLog = GetOutputLog(sw.Elapsed);
        }
        ...
        private static string GetOutputLog(TimeSpan ts)
        {
            string log = ProcessProgress.Current.GetDefaultOutput();

            string timeDesp = string.Format("经过时间{0:#,##0}秒", ts.TotalSeconds);

            if (log.Length > 7680)
                log = log.Substring(0, 7680) + "...";
