...
namespace PermissionCenter.Tables
{
    public class OguObjectTableBuilder : TableBuilderBase
    {
        ...
        protected override IList<string> GetReservedProperies()
        {
            ...
            reserved.Add("Mail");
            reserved.Add("UserRank");
            reserved.Add("Comment");

            return reserved;
        }

        protected void AddNotExistedProperty(IList<string> properties, string propertyName)
        {
            if (properties.Exists(p => string.Compare(p, propertyName, true) == 0) == false)
