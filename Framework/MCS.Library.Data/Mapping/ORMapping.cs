    ...
    public static partial class ORMapping
    {
        ...
        private static void SetValueToObject(ORMappingItem item, object graph, object data, object row, DataToObjectDeligations dod)
        {
            if (string.IsNullOrEmpty(item.SubClassPropertyName))
                SetMemberValueToObject(item, item.MemberInfo, graph, data);
            else
            {
                if (graph != null)
                {
                    ...
                    if (mi != null)
                    {
                        object subGraph = GetMemberValueFromObject(item, mi, graph);
                        ...
                        if (subGraph == null)
                        {
                            ...
                            SetMemberValueToObject(item, item.MemberInfo, subGraph, data);
                            SetMemberValueToObject(item, mi, graph, subGraph);
                        }
                        else
                            SetMemberValueToObject(item, item.MemberInfo, subGraph, data);
                    }
        ...
        {
            object data = GetMemberValueFromObject(item, item.MemberInfo, graph);
        ...
        private static void FillMappingItemByAttr(ORMappingItem item, ORFieldMappingAttribute fm)
        {
            ...
            item.UtcTimeToLocal = fm.UtcTimeToLocal;
        }
