...
FROM SC.UserAndContainerSnapshot UC (NOLOCK)
	...
	INNER JOIN SC.SchemaRelationObjectsSnapshot SR (NOLOCK) ON UC.UserID = SR.ObjectID INNER JOIN SC.SchemaObjectSnapshot SC ON SC.ID = SR.ObjectID
WHERE {0}