...
namespace MCS.Library.Office.OpenXml.Word.Test
{
    ...
    public class BuildDocumentTest
    {
        ...
        [TestMethod]
        [Description("根据书签与控件，复制多份模板，并且填写值")]
        [TestCategory("Word")]
        public void FillCopyPageTest()
        {
            DataTable dt = new DataTable();
            dt.Columns.Add("PeriodType", typeof(string));
            ...
            for (int i = 1; i <= 120; i++)
            {
                ...
                dr["d1"] = System.DateTime.Now.Day;

                dr["y2"] = System.DateTime.Now.Year + 1;
                dr["m2"] = System.DateTime.Now.Month;
                dr["d2"] = System.DateTime.Now.Day;

                dt.Rows.Add(dr);
            }

            SimplePropertyCollection properties = new SimplePropertyCollection();

            properties.Add(new DCTSimpleProperty() { TagID = "PeriodType", IsReadOnly = true });
            properties.Add(new DCTSimpleProperty() { TagID = "y1", IsReadOnly = true });
            properties.Add(new DCTSimpleProperty() { TagID = "m1", IsReadOnly = true });
            properties.Add(new DCTSimpleProperty() { TagID = "d1", IsReadOnly = true });

            properties.Add(new DCTSimpleProperty() { TagID = "y2", IsReadOnly = true });
            properties.Add(new DCTSimpleProperty() { TagID = "m2", IsReadOnly = true });
            properties.Add(new DCTSimpleProperty() { TagID = "d2", IsReadOnly = true });
            ...
            byte[] fillBinary = WordEntry.CopyPageFillData(templateBinary, dt.DefaultView, properties);
