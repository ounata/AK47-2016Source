...
namespace MCS.Library.Office.OpenXml.Word
{
    public class WordEntry
    {
        ...
        private static void CopyPageFillTemplate(Body wordBody, SimplePropertyCollection simpleProperties, Func<string, object> getValue)
        {
            foreach (SdtElement fillItem in wordBody.Descendants<SdtElement>())
            {
                foreach (SdtAlias sdtAlias in fillItem.Descendants<SdtAlias>())
                {
                    DCTSimpleProperty dcts = simpleProperties[sdtAlias.Val];
                    if (dcts != null)
                    {
                        ...
                        if (items.Count() > 0)
                        {
                            ...
                            runElement.AppendChild<Text>(new Text(GeneralFormatter.ToString(getValue(dcts.TagID), dcts.FormatString)));
        ...
        private static void ClonePageAndFill(DataRowView drv, OpenXmlElement item, ref OpenXmlElement addItem, SimplePropertyCollection simpleProperties)
        {
            foreach (SdtElement sdtElement in item.Descendants<SdtElement>())
            {
                foreach (SdtAlias ite11 in sdtElement.Descendants<SdtAlias>())
                {
                    DCTSimpleProperty property = simpleProperties[ite11.Val];
                    ...
                    if (property != null)
                    {
                        ...
                        if (items.Count() > 0)
                        {
                            Run runElement = items.First();
                            runElement.RemoveAllChildren<Text>();
                            runElement.AppendChild<Text>(new Text(GeneralFormatter.ToString(drv[property.TagID], property.FormatString)));
