...
namespace MCS.Library.Core
{
    ...
    public abstract class ActionContextBase<T> where T : ActionContextBase<T>, new()
    {
        ...
        public static T Current
        {
            get
            {
                T result = (T)ActionContextCache.Instance.GetOrAddNewValue(GetAndCheckContextKey(),
                        (cache, key) =>
                        {
                            T context = new T();
    ...
    internal sealed class ActionContextCache : ContextCacheQueueBase<object, object>
    {
        /// <summary>
        /// ObjectContextCache的实例，此处必须是属性，动态计算
        /// </summary>
        public static ActionContextCache Instance
        {
            get
            {
                return ContextCacheManager.GetInstance<ActionContextCache>();
            }
        }

        private ActionContextCache()
        {
        }
    }
}
