...
namespace PPTS.WebAPI.Products.ViewModels.Discounts
{
    public class CreateDiscountModel
    {
        ...
        public DiscountPermissionsApplieCollection DiscountPermissionsApplieCollection { get; set; }

        public DiscountItemCollection DiscountItemCollection { get; set; }

        public CheckResultModel CheckCreateDiscount() {
            ...
            if (DiscountPermissionsApplieCollection == null || DiscountPermissionsApplieCollection.Count == 0) {
                result.SetErrorMsg("折扣表主要内容不得为空，请填写后再保存！");
                return result;
            }

            if (DiscountItemCollection == null || DiscountItemCollection.Count == 0) {
                result.SetErrorMsg("折扣表主要内容不得为空，请填写后再保存！");
                return result;
            }
        ...
        public CheckResultModel CheckDiscountPermissionsApplieCollection() {
            ...
            foreach (var v in DiscountPermissionsApplieCollection)
                sb.AppendFormat(",'{0}'", v.CampusID);
            orgIDs = string.Format("({0})", sb.ToString().Substring(1));
            DiscountPermissionsApplieCollection coll = DiscountPermissionsApplyAdapter.Instance.LoadApprovingCollectionByOrgIDs(orgIDs);
            if (coll != null && coll.Count > 0)
            {
                ...
                result.SetErrorMsg(string.Format("{0}已有一个待审批的折扣表，请勿重复提交", sb.ToString().Substring(1)));
                return result;
            }
